...
{
    // PA2 USART2_TX, PA3 USART2_RX, AF07
...
#define usb_usart_is_rx_empty() ((USART2->SR & USART_SR_RXNE) != USART_SR_RXNE)
#define usb_usart_is_tx_empty() ((USART2->SR & USART_SR_TXE) == USART_SR_TXE)

void usb_usart_rx(uint8_t *data)
{
    *data = USART2->DR & 0xFF;
}

void usb_usart_tx(uint8_t *data)
{
    USART2->DR = *data;
    while (!usb_usart_is_tx_empty());
}

void usb_usart_tx_msg(uint8_t *data, uint8_t len)
{
    while (--len) {
        usb_usart_tx(data++);
    }
...
void main(void)
{
    ...
    uint16_t raw[10];
    uint8_t raw_index = 0;
    char msg[255]; // Overkill but we have the space!
    uint8_t data;
    while(1) {
        // Toggle the LED
        ...
        if (raw_index == 10) {
            ...
            // Transmit over USART to virtual COM port
            sprintf(msg, "Temp: %.1fC/%.1fF\t%.1f\r\n", cel, (cel*1.8)+32.0, average);
            usb_usart_tx_msg(msg, strlen(msg));
            raw_index = 0;
        }
        raw[raw_index++] = get_temperature_raw();
        // Slow echo back if we type something into the terminal
        if (!usb_usart_is_rx_empty()) {
            usb_usart_rx(&data);
            usb_usart_tx(&data);
        }
    }
